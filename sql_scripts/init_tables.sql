-- Base SQL script needed to start the project in a valid state
...
-- Need to insert history, so ORM knows which which migrations it has to start with
DELETE FROM `migratehistory`;
...
ALTER TABLE `game`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9694676;
