...
def migrate_forward(op, old_orm, new_orm):
    op.sql("ALTER TABLE `player` MODIFY COLUMN `name` VARCHAR(64) NOT NULL DEFAULT ''")
    op.sql("CREATE INDEX `player_name` ON `player` (`name`)")


def migrate_backward(op, old_orm, new_orm):
    op.sql("DROP INDEX `player_name` ON `player`")
    op.sql("ALTER TABLE `player` MODIFY COLUMN `name` TEXT NOT NULL DEFAULT ''")
