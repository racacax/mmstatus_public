...
def forward(old_orm, new_orm):
    ...
    return [
        # Apply default value datetime.datetime(1970, 1, 1, 1, 0) to the field player.last_name_update,
        player.update({player.last_name_update: datetime.datetime(1970, 1, 1, 1, 0)}).where(player.last_name_update.is_null(True)),
    ]
