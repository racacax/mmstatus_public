...
def backward(old_orm, new_orm):
    rankstatrollup = new_orm['rankstatrollup']
    return [
        # Apply default value datetime.datetime(1970, 1, 1, 1, 0) to the field rankstatrollup.last_game_time,
        rankstatrollup.update({rankstatrollup.last_game_time: datetime.datetime(1970, 1, 1, 1, 0)}).where(rankstatrollup.last_game_time.is_null(True)),
    ]
