...
class Player(peewee.Model):
    ...
    name = CharField(default='', index=True, max_length=64)
    ...
    region = snapshot.ForeignKeyField(backref='player_regions', index=True, model='zone', null=True)
...
class RankStatRollup(peewee.Model):
    ...
    last_game_time = DateTimeField(default=datetime.datetime(1970, 1, 1, 1, 0), index=True, null=True)
    class Meta:
...
def forward(old_orm, new_orm):
    ...
    return [
        # Apply default value 0 to the field player.points_fetch_retries,
        player.update({player.points_fetch_retries: 0}).where(player.points_fetch_retries.is_null(True)),
    ]
